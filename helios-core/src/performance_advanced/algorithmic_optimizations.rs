    ...
    F: Fn(&T, &T) -> Ordering,
{
    ...
    let _pivot_index = partition(arr, &compare);
    ...
    while let Some((start, end)) = stack.pop() {
        ...
        if is_sorted(&arr[start..end], &compare) {
            continue;
        }
        
        let pivot_idx = partition(&mut arr[start..end], &compare) + start;
...
fn fft_recursive(data: &mut [f64]) -> Vec<f64> {
    ...
    for i in 0..n/2 {
        ...
        let _t_imag = sin_val * odd_fft[i] + cos_val * 0.0;
