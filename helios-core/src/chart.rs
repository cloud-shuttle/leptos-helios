...
impl ChartSpec {
    ...
    pub fn optimize(self) -> Self {
        ...
        let optimized_transforms = self.transform.into_iter()
            .map(|t| t.optimize())
            .collect();
...
impl Encoding {
        ...
        match mark {
            MarkType::Point { .. } | MarkType::Scatter { .. } => {
                if self.x.is_none() || self.y.is_none() {
                    return Err(ValidationError::MissingRequiredEncoding("x and y required for points/scatter".to_string()));
                }
            },
            MarkType::Line { .. } => {
                if self.x.is_none() || self.y.is_none() {
                    return Err(ValidationError::MissingRequiredEncoding("x and y required for lines".to_string()));
                }
            },
            MarkType::Bar { .. } => {
                if self.x.is_none() || self.y.is_none() {
                    return Err(ValidationError::MissingRequiredEncoding("x and y required for bars".to_string()));
                }
            },
            MarkType::Area { .. } => {
                if self.x.is_none() || self.y.is_none() {
                    return Err(ValidationError::MissingRequiredEncoding("x and y required for areas".to_string()));
                }
            },
            MarkType::BoxPlot { .. } => {
                if self.x.is_none() || self.y.is_none() {
                    return Err(ValidationError::MissingRequiredEncoding("x and y required for box plots".to_string()));
                }
            },
            MarkType::Violin { .. } => {
                if self.x.is_none() || self.y.is_none() {
                    return Err(ValidationError::MissingRequiredEncoding("x and y required for violin plots".to_string()));
                }
            },
            MarkType::Heatmap { .. } => {
                if self.x.is_none() || self.y.is_none() {
                    return Err(ValidationError::MissingRequiredEncoding("x and y required for heatmaps".to_string()));
                }
            },
            MarkType::Histogram { .. } => {
                if self.x.is_none() {
                    return Err(ValidationError::MissingRequiredEncoding("x required for histograms".to_string()));
                }
            },
            MarkType::Density { .. } => {
                if self.x.is_none() {
                    return Err(ValidationError::MissingRequiredEncoding("x required for density plots".to_string()));
                }
            },
            MarkType::Contour { .. } => {
                if self.x.is_none() || self.y.is_none() {
                    return Err(ValidationError::MissingRequiredEncoding("x and y required for contour plots".to_string()));
                }
            },
            MarkType::Radar { .. } => {
                if self.x.is_none() || self.y.is_none() {
                    return Err(ValidationError::MissingRequiredEncoding("x and y required for radar charts".to_string()));
                }
            },
            MarkType::Sankey { .. } => {
                if self.x.is_none() || self.y.is_none() {
                    return Err(ValidationError::MissingRequiredEncoding("x and y required for sankey diagrams".to_string()));
                }
            },
            MarkType::Treemap { .. } => {
                if self.x.is_none() || self.y.is_none() {
                    return Err(ValidationError::MissingRequiredEncoding("x and y required for treemaps".to_string()));
                }
            },
            _ => {}
        }
    ...
    pub fn complexity(&self) -> f64 {
        ...
        if self.x.is_some() { complexity += 1.0; }
        if self.y.is_some() { complexity += 1.0; }
        if self.color.is_some() { complexity += 1.5; }
        if self.size.is_some() { complexity += 1.0; }
        if self.shape.is_some() { complexity += 1.0; }
        if self.facet.is_some() { complexity += 2.0; }
        complexity
    }
}
        ...
        match self {
            Transform::Filter { expression } => {
                if expression.is_empty() {
                    return Err(ValidationError::InvalidTransform("Empty filter expression".to_string()));
                }
            },
            Transform::Aggregate { operations, groupby } => {
                if operations.is_empty() {
                    return Err(ValidationError::InvalidTransform("No aggregation operations".to_string()));
                }
            },
            _ => {}
        }
...
impl Selection {
    pub fn validate(&self) -> Result<(), ValidationError> {
        if self.name.is_empty() {
            return Err(ValidationError::InvalidSelection("Empty selection name".to_string()));
        }
        Ok(())
    }
}
...
impl Intelligence {
    ...
    pub fn complexity(&self) -> f64 {
        let mut complexity = 0.0;
        if self.forecast.is_some() { complexity += 2.0; }
        if self.anomaly_detection.is_some() { complexity += 1.5; }
        if self.trend_analysis.unwrap_or(false) { complexity += 1.0; }
        if self.clustering.is_some() { complexity += 2.5; }
        complexity
    }
}
...
impl ForecastConfig {
    pub fn validate(&self) -> Result<(), ValidationError> {
        if self.periods == 0 {
            return Err(ValidationError::InvalidIntelligence("Forecast periods must be > 0".to_string()));
        }
        if let Some(confidence) = self.confidence {
            if confidence <= 0.0 || confidence >= 1.0 {
                return Err(ValidationError::InvalidIntelligence("Confidence must be between 0 and 1".to_string()));
            }
        }
        Ok(())
    }
...
impl AnomalyConfig {
    pub fn validate(&self) -> Result<(), ValidationError> {
        if self.threshold <= 0.0 || self.threshold >= 1.0 {
            return Err(ValidationError::InvalidIntelligence("Anomaly threshold must be between 0 and 1".to_string()));
        }
        Ok(())
    }
}
    ...
    pub fn validate(&self) -> Result<(), ValidationError> {
        match self {
            DataReference::DataFrame(df) => {
                if df.is_empty() {
                    return Err(ValidationError::DataValidation("Empty DataFrame".to_string()));
                }
            },
            ...
            DataReference::Query { sql, dataset } => {
                if sql.is_empty() || dataset.is_empty() {
                    return Err(ValidationError::DataValidation("Empty SQL query or dataset name".to_string()));
                }
            },
            _ => {}
        }
