...
use std::sync::Arc;
use crate::performance::{PerformanceManager, PerformanceConfig, CacheKey, SimdProcessor};
...
impl StrategySelector {
    ...
    pub fn select(&self, spec: &DataSpec) -> ProcessingStrategy {
        ...
        let avg_cpu_time = self.get_average_cpu_time(data_size);
        let avg_gpu_time = self.get_average_gpu_time(data_size);
...
impl DataProcessor {
    ...
    pub async fn process_cpu(&self, spec: &DataSpec, _config: &RayonConfig) -> Result<ProcessedData, DataError> {
        ...
        if !groupby_columns.is_empty() && !aggregate_operations.is_empty() {
            ...
            // Just grouping
            df = self.apply_aggregation(df, &Aggregation::GroupBy { columns: groupby_columns })?;
        } else if !aggregate_operations.is_empty() {
            // Just aggregation
            df = self.apply_aggregation(df, &Aggregation::Aggregate { operations: aggregate_operations })?;
        }
    ...
    pub async fn process_gpu(&self, spec: &DataSpec, _config: &ComputeConfig) -> Result<ProcessedData, DataError> {
        self.process_cpu(spec, &RayonConfig { num_threads: None, chunk_size: None, enable_simd: false }).await
    }
    ...
    async fn load_data(&self, source: &DataSource) -> Result<DataFrame, DataError> {
        match source {
            ...
            _ => Err(DataError::Processing("Only DataFrame sources supported in minimal implementation".to_string())),
        }
    }
    ...
    fn apply_filter(&self, df: DataFrame, filter: &Filter) -> Result<DataFrame, DataError> {
        match filter {
            ...
            Filter::Expression { expr } => {
                self.apply_expression_filter(df, expr)
            },
            Filter::Values { column, values } => {
                self.validate_column_exists(&df, column)?;
                let value_exprs: Vec<Expr> = values.iter().map(|v| {
                    match v {
    ...
    fn apply_expression_filter(&self, df: DataFrame, expr: &str) -> Result<DataFrame, DataError> {
        ...
        if expr.contains("SELECT") || expr.contains("FROM") || expr.contains("WHERE") {
            return Err(DataError::Expression("SQL expressions not supported in filter context".to_string()));
        }
    ...
    fn convert_data_type(&self, data_type: &DataType) -> Result<polars::prelude::DataType, DataError> {
        match data_type {
            ...
            DataType::DateTime => Ok(polars::prelude::DataType::Datetime(TimeUnit::Milliseconds, None)),
        }
    }
    ...
    fn apply_grouped_aggregation(&self, df: DataFrame, groupby_columns: &[String], operations: &[AggOp]) -> Result<DataFrame, DataError> {
...
impl DataProcessor {
    ...
    fn apply_transform_optimized(&self, df: DataFrame, transform: &DataTransform) -> Result<DataFrame, DataError> {
        let _timer = self.performance_manager.profiler().start_timer("apply_transform_optimized".to_string());
        ...
        match transform {
            ...
            DataTransform::Cast { column, data_type } => {
                ...
                let lazy_df = df.lazy().with_columns([
                    col(column).cast(polars_type)
                ]);
                Ok(lazy_df.collect()?)
            },
            DataTransform::FillNull { column, value } => {
                ...
                let lazy_df = match value {
                    ...
                    FillValue::Mean => df.lazy().with_columns([col(column).fill_null(col(column).mean())]),
    ...
    fn apply_filter_optimized(&self, df: DataFrame, filter: &Filter) -> Result<DataFrame, DataError> {
        let _timer = self.performance_manager.profiler().start_timer("apply_filter_optimized".to_string());
        ...
        match filter {
            ...
            Filter::Range { column, min, max } => {
                ...
                let lazy_df = df.lazy().filter(
                    col(column).gt(lit(min_val)).and(col(column).lt(lit(max_val)))
                );
                Ok(lazy_df.collect()?)
            },
            ...
            Filter::Null { column, keep_nulls: _ } => {
                self.validate_column_exists(&df, column)?;
    ...
    fn apply_aggregation_optimized(&self, df: DataFrame, aggregation: &Aggregation) -> Result<DataFrame, DataError> {
        let _timer = self.performance_manager.profiler().start_timer("apply_aggregation_optimized".to_string());
        
        match aggregation {
            Aggregation::Aggregate { operations } => {
                self.apply_aggregation(df, aggregation)
            },
            Aggregation::GroupBy { columns } => {
                self.apply_grouped_aggregation(df, columns, &[])
            },
            Aggregation::Window { operations } => {
                ...
                for op in operations {
                    ...
                    match op {
                        WindowOp::RollingSum { column, window: _, alias } => {
                            lazy_df = lazy_df.with_columns([
                                col(column).sum().alias(alias)
                            ]);
                        },
                        WindowOp::RollingMean { column, window: _, alias } => {
                            lazy_df = lazy_df.with_columns([
                                col(column).mean().alias(alias)
                            ]);
                        },
