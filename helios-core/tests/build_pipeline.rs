...
fn test_trunk_availability() {
    let output = Command::new("trunk").arg("--version").output();

    match output {
        Ok(result) => {
            assert!(
                result.status.success(),
                "Trunk should be available and executable"
            );
            let version = String::from_utf8_lossy(&result.stdout);
            assert!(
                version.contains("trunk"),
                "Trunk version output should contain 'trunk'"
            );
        }
        Err(_) => {
...
fn test_wasm_pack_availability() {
    ...
    match output {
        Ok(result) => {
            assert!(
                result.status.success(),
                "wasm-pack should be available and executable"
            );
            let version = String::from_utf8_lossy(&result.stdout);
            assert!(
                version.contains("wasm-pack"),
                "wasm-pack version output should contain 'wasm-pack'"
            );
        }
        Err(_) => {
...
fn test_wasm_opt_availability() {
    ...
    match output {
        Ok(result) => {
            assert!(
                result.status.success(),
                "wasm-opt should be available and executable"
            );
            let version = String::from_utf8_lossy(&result.stdout);
            assert!(
                version.contains("wasm-opt"),
                "wasm-opt version output should contain 'wasm-opt'"
            );
        }
        Err(_) => {
...
fn test_trunk_config_structure() {
    ...
    if config_path.exists() {
        let content = fs::read_to_string(config_path).expect("Should be able to read Trunk.toml");

        // Basic validation of Trunk configuration
        assert!(
            content.contains("[build]"),
            "Trunk config should have [build] section"
        );
        assert!(
            content.contains("target"),
            "Trunk config should specify target"
        );
        assert!(
            content.contains("dist"),
            "Trunk config should specify dist directory"
        );
    } else {
...
fn test_build_script_structure() {
    ...
    if build_script.exists() {
        let content = fs::read_to_string(build_script).expect("Should be able to read build.sh");

        // Basic validation of build script
        assert!(
            content.contains("#!/bin/bash"),
            "Build script should start with shebang"
        );
        assert!(content.contains("cargo"), "Build script should use cargo");
        assert!(
            content.contains("wasm-pack"),
            "Build script should use wasm-pack"
        );
    } else {
...
fn test_dev_server_config() {
    ...
    if config_path.exists() {
        ...
        assert!(
            content.contains("[serve]") || content.contains("serve"),
...
fn test_wasm_optimization_config() {
    ...
    if config_path.exists() {
        ...
        assert!(
            content.contains("optimize") || content.contains("wasm-opt"),
