...
fn test_data_processor_creation() {
    ...
    assert!(processor.is_ok(), "DataProcessor should be created successfully");
}
...
fn test_strategy_selection_for_streaming_data() {
    ...
    let selector = StrategySelector::new();
    let spec = DataSpec {
        source: DataSource::Stream { stream_id: "test_stream".to_string() },
        transforms: vec![],
        filters: vec![],
        aggregations: vec![],
        output_format: OutputFormat::DataFrame,
...
fn test_data_spec_complexity_calculation() {
    ...
    let complex_spec = DataSpec {
        source: DataSource::DataFrame(create_test_dataframe()),
        transforms: vec![
            DataTransform::Select { columns: vec!["x".to_string(), "y".to_string()] },
            DataTransform::Rename { mappings: [("x".to_string(), "x_coord".to_string())].iter().cloned().collect() },
        ],
        filters: vec![
            Filter::Range { column: "x".to_string(), min: Some(0.0), max: Some(100.0) },
            Filter::Values { column: "category".to_string(), values: vec![serde_json::Value::String("A".to_string())] },
        ],
        aggregations: vec![
            Aggregation::GroupBy { columns: vec!["category".to_string()] },
            Aggregation::Aggregate { operations: vec![
                AggOp::Sum { column: "y".to_string(), alias: Some("total_y".to_string()) },
                AggOp::Mean { column: "y".to_string(), alias: Some("avg_y".to_string()) },
            ]},
        ],
        output_format: OutputFormat::DataFrame,
    };
    ...
    assert_eq!(simple_complexity, 1.0, "Simple spec should have complexity 1.0");
    assert!(complex_complexity > simple_complexity, "Complex spec should have higher complexity");
}
...
fn test_data_spec_size_estimation() {
    ...
    assert!(small_size < large_size, "Large spec should have larger estimated size");
...
fn test_data_spec_streaming_detection() {
    ...
    let streaming_spec = DataSpec {
        source: DataSource::Stream { stream_id: "test".to_string() },
        transforms: vec![],
        filters: vec![],
        aggregations: vec![],
        output_format: OutputFormat::DataFrame,
    ...
    assert!(streaming_spec.is_streaming(), "Streaming spec should be detected as streaming");
    assert!(!non_streaming_spec.is_streaming(), "Non-streaming spec should not be detected as streaming");
}
...
fn test_data_spec_hashing() {
    ...
    let spec3 = DataSpec {
        source: DataSource::DataFrame(create_test_dataframe()),
        transforms: vec![DataTransform::Select { columns: vec!["x".to_string()] }],
        filters: vec![],
        aggregations: vec![],
        output_format: OutputFormat::DataFrame,
    };
...
async fn test_data_processor_cpu_processing() {
    ...
    let result = processor.process_cpu(&spec, &RayonConfig { num_threads: None, chunk_size: None, enable_simd: false }).await;
    assert!(result.is_ok(), "CPU processing should succeed");
    
    let processed_data = result.unwrap();
    assert_eq!(processed_data.data.height(), 5, "Processed data should have 5 rows");
    assert_eq!(processed_data.data.width(), 3, "Processed data should have 3 columns");
}
...
async fn test_data_processor_with_filters() {
    ...
    let spec = DataSpec {
        ...
        transforms: vec![],
        filters: vec![
            Filter::Range { column: "x".to_string(), min: Some(2.0), max: Some(4.0) },
        ],
        aggregations: vec![],
        output_format: OutputFormat::DataFrame,
    };
    ...
    let processed_data = result.unwrap();
    assert!(processed_data.data.height() < 5, "Filtered data should have fewer rows");
}
...
async fn test_data_processor_with_transforms() {
    ...
    let processed_data = result.unwrap();
    assert_eq!(processed_data.data.width(), 2, "Transformed data should have 2 columns");
    let column_names: Vec<String> = processed_data.data.get_column_names().iter().map(|s| s.to_string()).collect();
...
async fn test_data_processor_with_aggregations() {
    ...
    let spec = DataSpec {
        ...
        aggregations: vec![
            ...
            Aggregation::Aggregate { operations: vec![
                AggOp::Sum { column: "y".to_string(), alias: Some("total_y".to_string()) },
                AggOp::Count { column: "y".to_string(), alias: Some("count".to_string()) },
            ]},
        ],
        output_format: OutputFormat::DataFrame,
    };
    ...
    let processed_data = result.unwrap();
    assert!(processed_data.data.height() <= 3, "Aggregated data should have fewer rows (grouped by category)");
    let column_names: Vec<String> = processed_data.data.get_column_names().iter().map(|s| s.to_string()).collect();
...
fn test_stream_buffer_creation() {
    ...
    assert_eq!(health.buffer_utilization, 0.0, "New buffer should have 0% utilization");
    assert_eq!(health.dropped_messages, 0, "New buffer should have 0 dropped messages");
    assert_eq!(health.current_size, 0, "New buffer should have 0 current size");
}
...
fn test_stream_buffer_push_and_process() {
    ...
    assert_eq!(health.buffer_utilization, 0.2, "Buffer should have 20% utilization");
    ...
    let processed_data = result.unwrap();
    assert_eq!(processed_data.data.height(), 5, "Processed batch should have 5 rows");
}
...
fn test_stream_buffer_overflow() {
    ...
    assert_eq!(health.dropped_messages, 1, "Buffer should have dropped 1 message");
    assert_eq!(health.buffer_utilization, 1.0, "Buffer should be at 100% utilization");
}
...
fn test_data_metadata_creation() {
    ...
    assert_eq!(metadata.row_count, 5, "Metadata should have correct row count");
    assert_eq!(metadata.column_count, 3, "Metadata should have correct column count");
    assert_eq!(metadata.column_types.len(), 3, "Metadata should have 3 column types");
    assert!(metadata.memory_usage > 0, "Metadata should have positive memory usage");
...
fn test_device_capabilities_detection() {
    ...
    assert!(capabilities.cpu_cores > 0, "Should detect at least 1 CPU core");
    assert!(capabilities.memory_gb > 0.0, "Should detect positive memory");
...
async fn test_end_to_end_data_processing() {
    ...
    let spec = DataSpec {
        ...
        transforms: vec![
            DataTransform::Select { columns: vec!["x".to_string(), "y".to_string(), "category".to_string()] },
            DataTransform::Cast { column: "x".to_string(), data_type: helios_core::data::DataType::Float64 },
        ],
        filters: vec![
            Filter::Range { column: "x".to_string(), min: Some(0.0), max: Some(100.0) },
        ],
    ...
    let processed_data = result.unwrap();
    assert!(processed_data.data.height() > 0, "Processed data should not be empty");
    assert!(processed_data.data.height() <= 10, "Aggregated data should have fewer rows");
    assert!(processed_data.processing_time > StdDuration::from_millis(0), "Should have processing time");
