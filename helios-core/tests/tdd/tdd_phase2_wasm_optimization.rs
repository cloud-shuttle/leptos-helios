...
mod tree_shaking_tests {
    ...
    fn test_tree_shaking_optimization() {
        ...
        assert!(
            optimized_efficiency >= initial_efficiency,
...
mod code_splitting_tests {
    ...
    fn test_code_chunk_creation() {
        ...
        for chunk in &chunks {
            assert!(
                chunk.size > 0,
                "Chunk '{}' should have positive size",
                chunk.name
            );
            assert!(
                chunk.size < 50 * 1024,
                "Chunk '{}' size {} should be < 50KB",
                chunk.name,
                chunk.size
            );
        }
...
mod performance_tests {
    ...
    fn test_optimization_performance() {
        ...
        assert!(
            ...
            "Optimization took {:?}, should be < 100ms",
            duration
        );
    }
    ...
    fn test_bundle_analysis_performance() {
        ...
        assert!(
            ...
            "Bundle analysis took {:?}, should be < 50ms",
            duration
        );
    }
    ...
    fn test_chunk_creation_performance() {
        ...
        assert!(
            ...
            "Chunk creation took {:?}, should be < 10ms",
            duration
        );

        // Should create reasonable number of chunks
        assert!(
            chunks.len() >= 3 && chunks.len() <= 10,
            "Should create 3-10 chunks, got {}",
            chunks.len()
        );
    }
}
...
mod integration_tests {
    ...
    fn test_end_to_end_wasm_optimization() {
        ...
        assert!(
            ...
            "Tree-shaking efficiency should improve"
        );

        // Verify size target
        let target_size = 120 * 1024; // 120KB
        assert!(
            engine.meets_size_target(target_size),
            "Should meet size target"
        );
